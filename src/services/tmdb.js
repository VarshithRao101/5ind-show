...
// 1. Point to your local proxy instead of TMDB directly
const tmdb = axios.create({
  baseURL: "/api/tmdb", // Calls your Vercel Middleware
  params: {
    language: 'en-US',
  },
  timeout: 15000,
...
// 2. Interceptor: Transform /movie/123 -> /api/tmdb?endpoint=/movie/123
tmdb.interceptors.request.use((config) => {
  if (config.url) {
    config.params = config.params || {};
    // Move the requested path (e.g., /movie/popular) to the 'endpoint' param
    config.params.endpoint = config.url;
    // Clear the url so axios sends request to baseURL only (+ params)
    config.url = '';
  }
  return config;
});
