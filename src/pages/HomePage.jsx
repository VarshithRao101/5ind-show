                ...
                <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex flex-col justify-end md:justify-center h-full">
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2 sm:gap-4 pt-2 sm:pt-4 md:pt-6">
                            ...
                            >
                                <FiPlay size={16} fill="black" className="md:w-5 md:h-5" />
                                Play
                            </button>

                            <button
                                onClick={() => navigate(movie.media_type === 'tv' ? `/tv/${movie.id}` : `/movie/${movie.id}`)}
                                className="flex items-center gap-2 md:gap-3 bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 sm:px-6 md:px-8 py-2 md:py-3.5 rounded-lg sm:rounded-xl hover:bg-white/20 transition-all font-bold text-xs sm:text-sm md:text-lg active:scale-95"
                            >
                                <FiInfo size={16} className="md:w-5 md:h-5" />
                                Info
                            </button>
                            ...
                            <button
                                ...
                                className="p-2 sm:p-2.5 md:p-3.5 rounded-full border border-white/20 bg-white/10 text-white hover:bg-white/20 transition-colors backdrop-blur-sm"
                                title={inList ? "Remove from List" : "Add to List"}
                            >
                                {inList ? <FiCheck size={18} className="md:w-5 md:h-5" /> : <FiPlus size={18} className="md:w-5 md:h-5" />}
                            </button>
                        </div>
                    </motion.div>
                </div>
...
export default function HomePage() {
    ...
    // Safety Timeout - Extended for slow networks
    useEffect(() => {
    ...
    const loadMovies = async () => {
        ...
        try {
            ...
            // We use localStorage instead of sessionStorage so data survives browser restarts/bad network
            const cached = localStorage.getItem('homeData_v2');
            if (cached) {
                const data = JSON.parse(cached);
                // Valid for 24 hours now to support "Download at College, Watch at Home" style usage
                if (Date.now() - data.timestamp < 24 * 60 * 60 * 1000) {
                    ...
                    // If we have data, we stop loading immediately, but we can still fetch fresh in bg (optional)
                    // For now, trust cache to prevent "crashing" feel.
                    setLoading(false);

                    // Optional: If cache is older than 1 hour, try to refresh efficiently
            ...
            // Only update if we got actual data (prevent overwriting good cache with bad empty data)
            const hasData = (trend?.length > 0 || ind?.length > 0);
            ...
            if (hasData) {
                ...
                // Cache Update (Persistent)
                localStorage.setItem('homeData_v2', JSON.stringify({
                    timestamp: Date.now(),
                    trending: trend || [],
                    indiaTop: ind || [],
                    topRated: top || [],
            ...
            console.warn("Home fetch failed, using fallback if available");
        } finally {
            setLoading(false);
        }
    };
    ...
    const EmptyState = ({ message, canRetry }) => (
        <div className="py-12 flex flex-col items-center justify-center text-gray-500 bg-white/5 rounded-xl mx-6 md:mx-12 border border-white/5">
            ...
            <p className="font-medium text-lg mb-4">{message}</p>
            {canRetry && (
                <button
                    onClick={loadMovies}
                    className="px-6 py-2 bg-[#FFD400] text-black font-bold rounded-lg hover:scale-105 transition-transform"
                >
                    Retry Connection
                </button>
            )}
